...
const TimelineItem = ({ event, coupleData, confirmDeleteId, setConfirmDeleteId, deleteEvent, startEdit, index, isLast }) => {
    ...
    const getIcon = (emotion) => {
        const iconStyle = { stroke: 'var(--color-primary)', strokeWidth: 2 };
        switch (emotion) {
            case 'heart': return <Heart size={16} style={iconStyle} />;
            case 'star': return <Star size={16} style={iconStyle} />;
            case 'smile': return <Smile size={16} style={iconStyle} />;
            case 'sun': return <Sun size={16} style={iconStyle} />;
